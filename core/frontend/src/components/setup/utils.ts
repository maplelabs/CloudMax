import { ApiConfigResponse, SetupConfig } from "./../../types/config";
...
export const generateConfigHash = (config: SetupConfig): string => {
  const normalizeConfig = (config: SetupConfig) => {
    return {
      ...
      // Include all other fields that can be modified
      triageLevels: config.triageLevels,
      orchestratorAgents: config.orchestratorAgents,
      automaticTriage: config.automaticTriage,
      alertGroupingEnabled: config.alertGroupingEnabled
    };
  };
