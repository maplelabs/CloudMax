...
export function LLMConfigurationModal({ llmConfig, onUpdate }: LLMConfigurationModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpen}>
      <DialogTrigger asChild>
        <Card className="box-shadow cursor-pointer hover:shadow-md transition-shadow">
          <CardHeader className="flex flex-row items-center justify-between py-4">
            <CardTitle className="">Primary LLM Configuration</CardTitle>
            <Button
              variant="ghost"
              size="sm"
            >
      ...
      <DialogContent className=" max-h-[90vh] overflow-y-auto" style={{ width: '774px' }}>
        <DialogHeader>
          <DialogTitle className="triage-journey-header">LLM Configuration</DialogTitle>
          <DialogDescription>
            Configure the Large Language Model provider for your agents.
          </DialogDescription>
        </DialogHeader>
        <div className="py-4">
          <LLMConfiguration
            llmConfig={tempConfig}
