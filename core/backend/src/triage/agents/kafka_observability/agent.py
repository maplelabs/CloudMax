...
from src.triage.agents.kafka_observability import prompts


class KafkaObservabilityAgent(AbstractAgent):
    """Kafka specialist React Agent with the ability to query observability tools for Kafka-specific investigations"""

    def get_supported_observability_systems(self) -> List[ObservabilitySystem]:
        """Kafka agent needs Grafana for metrics and OpenSearch for logs."""
        return [ObservabilitySystem.GRAFANA, ObservabilitySystem.OPENSEARCH]

    async def get_mcp_config(self) -> Dict[str, Any]:
        """
        Get the MCP configuration for Kafka Agent.
        Only needs Grafana for Kafka metrics.
    ...
    def get_tools_needed(self) -> List[BaseTool]:
        """
        Get the tools needed by the Kafka Agent.
    ...
    def get_agent_name(self) -> str:
        ...
        Returns:
            str: The agent name
        """
        return "Kafka_Observability_Agent"
